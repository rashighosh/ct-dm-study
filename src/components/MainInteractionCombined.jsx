...
import '../css/MainInteractionCombined.css'
...
export default function MainInteraction() {
  ...
  const [jordanConversationModel, setJordanConversationModel] = useState(() => {
    ...
    if (Array.isArray(savedModel?.themes)) {
      const cleanedThemes = savedModel.themes.map((theme) => {
        const { summary, ...rest } = theme

        return {
          ...rest,
          takeaway: theme.takeaway ?? summary ?? '',
      ...
      return {
        working_hypothesis: savedModel.working_hypothesis ?? '',
  ...
  async function handleWorkspaceToggle() {
    ...
    if (
      willOpen &&
      isDistributedWorkspaceCondition &&
      jordanGuidance &&
      !jordanGuidance.hasSpoken
    ) {
      setIsJordanWorkspaceOpen(true)
      setIsSensemakingActive(true)
      setIsJordanActive(true)
      ...
      try {
        ...
        console.error('Jordan workspace speech failed:', error)
      } finally {
        setJordanSubtitle('')
        setIsJordanActive(false)
        playGesture('stopCompanionGesture')
    ...
    try {
      ...
      if (jordanTurnData) {
        ...
        if (
          ...
          playGesture('stopCompanionGesture')
          playGesture('startWriting')
          setTimeout(() => {
            setJordanConversationModel({
              working_hypothesis: jordanTurnData.working_hypothesis ?? '',
        ...
        const guidanceWithId = {
          id: uid(),
          update_type: jordanTurnData.update_type,
          guidance_type: jordanTurnData.update_type,
...
function LiveJordanWorkspace({
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        list.scrollTo({
          top: Math.max(note.offsetTop - 8, 0),
          behavior: 'smooth',
        })
      })
    })
  ...
  return (
    ...
    >
      ...
      {themes.length > 0 && (
        <div ref={themeListRef} className="jordan-live-theme-list">
          {themes.slice(0, 3).map((theme) => {
            const newestDetail =
              theme.details?.[theme.details.length - 1] || null
            const showNewestDetail = (theme.details?.length || 0) > 1

            const isCurrentNewDetail = guidance?.detail_id === newestDetail?.id
              ...
              <div className="jordan-live-theme-row" key={theme.id}>
                ...
                >
                  <div className="jordan-live-detail-list">
                    <div className="jordan-live-detail jordan-live-takeaway">
                      <div className="jordan-live-detail-static">
                        <span>{theme.takeaway || 'Key takeaway'}</span>
                      </div>
                    </div>

                    {showNewestDetail && newestDetail && (
                      <div
                        className={`jordan-live-new-piece ${
                          isCurrentNewDetail ? 'is-new' : ''
                        }`}
                      >
                        {isCurrentNewDetail && (
                          <span className="jordan-live-new-piece-label">
                            + New
                          </span>
                        )}

                        <span>{newestDetail.text}</span>
                      </div>
                    )}
                  </div>
                </section>
                ...
                <div className="jordan-live-theme-title">
                  <div className="jordan-live-theme-static">
...
function SensemakingExplorationCard({ conversationModel, onDismiss }) {
  const themes = conversationModel?.themes || []
  const latestConnection = conversationModel?.latestConnection || null
        ...
        {themes.length === 0 ? (
          ...
          themes.map((theme) => (
            <section className="jordan-workspace-drawer-theme" key={theme.id}>
              <div className="jordan-workspace-drawer-theme-title is-static">
                <strong>{theme.label || 'Theme'}</strong>
              </div>

              <div className="jordan-workspace-drawer-notes">
                <div className="jordan-workspace-drawer-note is-static">
                  <p>{theme.takeaway}</p>
                </div>
              </div>
            </section>
